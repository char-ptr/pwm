...
import { searchItemQuery, selectedItem } from "@/lib/state/app";
...
export function ClientVaultItem({ item }: Props) {
  ...
  const [search, _] = useAtom(searchItemQuery);
  return (
    decrypted_item?.name.includes(search) && (
      <>
        <button
          type="button"
          onClick={select_item}
          className={`flex rounded-lg flex-row gap-4 items-center p-4 hover:bg-neutral-600/40 ${aselected_item?.item_id === decrypted_item.item_id &&
